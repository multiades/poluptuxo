...
document.addEventListener('DOMContentLoaded', () => { 
  ...
  // Select every navbar child element apart from the toggler label
  const children = navbar.querySelectorAll(':scope > *:not(label[for="navbar-toggle"])');
  ...
  navbar.querySelector('div#trap').addEventListener('focus', () => { // Run once so as not to add many listeners
    navbar.querySelector('label[for="navbar-toggle"]').focus();
  });
